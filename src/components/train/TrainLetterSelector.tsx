...
import { getTrainingData, clearTrainingData } from '@/services/trainingService';
...
const TrainLetterSelector: React.FC<TrainLetterSelectorProps> = ({ selectedLetter, onSelectLetter }) => {
      ...
      if (target.files && target.files.length > 0) {
        ...
        reader.onload = (event) => {
          try {
            ...
            localStorage.setItem('asl-training-data', JSON.stringify(jsonData));
            toast.success("Training data imported successfully", {
              description: `${jsonData.length} letter datasets imported`,
            });
          } catch (error) {
  ...
  const handleExportData = () => {
    ...
    link.href = url;
    link.download = 'asl-training-data.json';
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    
    toast.success("Training data exported", {
      description: `${data.length} letter datasets exported`,
    });
  };
